// Derived from DLSS5-Feeder commit 792755324574ff4703eb441a7bc14c724e125b84:
// https://github.com/jlrouzies-fr/DLSS5-Feeder (MIT; see LICENSE-DLSS5-Feeder).
// WanGP adds --wangp-video: streaming RGBA8 + R16G16_FLOAT motion + R32_FLOAT depth.
//
...
#include "feed_ipc.h"
#include "feed_fmt.h"
#include "feed_dfc.h"   // Deep Fried Chicken interop ABI 1 (producer side, HostMode=1 here)
...
static bool g_stdio_mode = false;
...
static void Log(const char *fmt, ...)
{
    ...
    SYSTEMTIME st;
    GetLocalTime(&st);
    fprintf(g_stdio_mode ? stderr : stdout, "%02u:%02u:%02u.%03u  %s\n", st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, line);
    FILE *f = nullptr;
...
static bool InitNgx()
{
    wchar_t data_path[MAX_PATH] = {}, feature_path[MAX_PATH] = {}, candidate[MAX_PATH] = {};
    GetModuleFileNameW(nullptr, data_path, MAX_PATH);
    if (wchar_t *s = wcsrchr(data_path, L'\\'))
    {
        *(s + 1) = L'\0';
        wcscpy_s(candidate, data_path);
        wcscat_s(candidate, L"..\\dlss\\");
    }
    const DWORD feature_path_length = GetFullPathNameW(candidate, MAX_PATH, feature_path, nullptr);
    if (feature_path_length == 0 || feature_path_length >= MAX_PATH) return false;
    const wchar_t *feature_paths[] = {feature_path};
    NVSDK_NGX_FeatureCommonInfo feature_info = {};
    feature_info.PathListInfo.Path = feature_paths;
    feature_info.PathListInfo.Length = 1;
    Log("[host] NGX feature search path: %ls", feature_path);

    NVSDK_NGX_Result r = NVSDK_NGX_D3D12_Init(0x1000000ULL, data_path, h.dev, &feature_info, NVSDK_NGX_Version_API);
    ...
    if (NVSDK_NGX_FAILED(r))
    {
        r = NVSDK_NGX_D3D12_Init_with_ProjectID("a0f57b54-1daf-4934-90ae-c4035c19df04", NVSDK_NGX_ENGINE_TYPE_CUSTOM,
                                                "1.0", data_path, h.dev, &feature_info, NVSDK_NGX_Version_API);
...
// ---------------------------------------------------------------------------
// WanGP stdin/stdout video protocol. Unlike the legacy direct feature-18 worker,
// this drives the public DLSS contract so the NR add-on sees real depth and motion.
// ---------------------------------------------------------------------------

static constexpr uint32_t WANGP_SETUP_MAGIC = 0x3144574Eu; // "NWD1"
static constexpr uint32_t WANGP_SETUP_OUT_MAGIC = 0x3152574Eu; // "NWR1"
static constexpr uint32_t WANGP_FRAME_MAGIC = 0x314D5246u; // "FRM1"
static constexpr uint32_t WANGP_FRAME_OUT_MAGIC = 0x3154554Fu; // "OUT1"

#pragma pack(push, 1)
struct WanGPVideoHeader
{
    uint32_t magic, width, height, output_width, output_height, frame_count;
    uint32_t flags, warmup, reserved0, reserved1;
};
struct WanGPSetupResult { uint32_t magic, status, width, height, output_width, output_height; };
struct WanGPFrameHeader { uint32_t magic, index, reset, reserved; int64_t pts; };
struct WanGPFrameResult { uint32_t magic, index, ok, bytes, ngx_result; int64_t pts; };
#pragma pack(pop)
...
static bool ReadStdio(void *data, size_t size)
{
...
static bool WriteStdio(const void *data, size_t size)
{
...
static D3D12_RESOURCE_BARRIER WanGPTransition(ID3D12Resource *resource, D3D12_RESOURCE_STATES before, D3D12_RESOURCE_STATES after)
{
...
static bool CreateWanGPTex(WanGPTex &item, UINT width, UINT height, DXGI_FORMAT format, UINT packed_pitch)
{
    ...
    if (FAILED(h.dev->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &texture, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&item.texture)))) return false;
    UINT rows = 0;
    UINT64 row_size = 0, total = 0;
    h.dev->GetCopyableFootprints(&texture, 0, 1, 0, &item.footprint, &rows, &row_size, &total);
    ...
    return SUCCEEDED(h.dev->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &buffer, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&item.upload)));
}

static bool CreateWanGPResources(WanGPVideoState &state, const WanGPVideoHeader &header)
{
    state.width = header.width;
    state.height = header.height;
    state.output_width = header.output_width;
    state.output_height = header.output_height;
    if (!CreateWanGPTex(state.color, state.width, state.height, DXGI_FORMAT_R8G8B8A8_UNORM, state.width * 4) ||
        !CreateWanGPTex(state.motion, state.width, state.height, DXGI_FORMAT_R16G16_FLOAT, state.width * 4) ||
        !CreateWanGPTex(state.depth, state.width, state.height, DXGI_FORMAT_R32_FLOAT, state.width * 4)) return false;

    D3D12_HEAP_PROPERTIES heap = {};
    heap.Type = D3D12_HEAP_TYPE_DEFAULT;
    D3D12_RESOURCE_DESC texture = {};
    texture.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
    texture.Width = state.output_width;
    texture.Height = state.output_height;
    texture.DepthOrArraySize = 1;
    texture.MipLevels = 1;
    texture.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    texture.SampleDesc.Count = 1;
    texture.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
    if (FAILED(h.dev->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &texture, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&state.output)))) return false;
    UINT rows = 0;
    UINT64 row_size = 0, total = 0;
    h.dev->GetCopyableFootprints(&texture, 0, 1, 0, &state.output_footprint, &rows, &row_size, &total);
    ...
    buffer.SampleDesc.Count = 1;
    buffer.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
    return SUCCEEDED(h.dev->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &buffer, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&state.readback)));
}

static bool FillWanGPUpload(WanGPTex &item, const void *data, UINT pitch, UINT height)
{
    BYTE *mapped = nullptr;
    if (FAILED(item.upload->Map(0, nullptr, reinterpret_cast<void **>(&mapped)))) return false;
    const BYTE *source = static_cast<const BYTE *>(data);
    for (UINT row = 0; row < height; ++row)
        memcpy(mapped + static_cast<size_t>(row) * item.footprint.Footprint.RowPitch, source + static_cast<size_t>(row) * pitch, pitch);
    item.upload->Unmap(0, nullptr);
    return true;
}

static void CopyWanGPUpload(WanGPTex &item)
{
    ...
    h.list->CopyTextureRegion(&destination, 0, 0, 0, &source, nullptr);
}

static bool UploadWanGPFrame(WanGPVideoState &state, const BYTE *color, const BYTE *motion, const BYTE *depth)
{
    if (!FillWanGPUpload(state.color, color, state.width * 4, state.height) ||
        !FillWanGPUpload(state.motion, motion, state.width * 4, state.height) ||
        !FillWanGPUpload(state.depth, depth, state.width * 4, state.height) || !BeginCommands()) return false;
    WanGPTex *items[] = { &state.color, &state.motion, &state.depth };
    if (state.inputs_ready)
        for (WanGPTex *item : items)
        {
            D3D12_RESOURCE_BARRIER barrier = WanGPTransition(item->texture, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COPY_DEST);
            h.list->ResourceBarrier(1, &barrier);
        }
    for (WanGPTex *item : items) CopyWanGPUpload(*item);
    for (WanGPTex *item : items)
    {
        D3D12_RESOURCE_BARRIER barrier = WanGPTransition(item->texture, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        h.list->ResourceBarrier(1, &barrier);
    }
...
static bool DownloadWanGPFrame(WanGPVideoState &state, std::vector<BYTE> &packed)
{
    if (!BeginCommands()) return false;
    D3D12_RESOURCE_BARRIER before = WanGPTransition(state.output, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE);
    h.list->ResourceBarrier(1, &before);
    D3D12_TEXTURE_COPY_LOCATION source = {}, destination = {};
    source.pResource = state.output;
    source.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX;
    destination.pResource = state.readback;
    destination.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
    destination.PlacedFootprint = state.output_footprint;
    h.list->CopyTextureRegion(&destination, 0, 0, 0, &source, nullptr);
    D3D12_RESOURCE_BARRIER after = WanGPTransition(state.output, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    h.list->ResourceBarrier(1, &after);
    const UINT64 fence = EndCommands();
    if (!WaitFenceValue(h.fence, fence, 60000)) return false;
    BYTE *mapped = nullptr;
    const UINT pitch = state.output_width * 4;
    D3D12_RANGE range = { 0, static_cast<SIZE_T>(state.output_footprint.Footprint.RowPitch) * state.output_height };
    if (FAILED(state.readback->Map(0, &range, reinterpret_cast<void **>(&mapped)))) return false;
    packed.resize(static_cast<size_t>(pitch) * state.output_height);
    for (UINT row = 0; row < state.output_height; ++row)
        memcpy(packed.data() + static_cast<size_t>(row) * pitch, mapped + static_cast<size_t>(row) * state.output_footprint.Footprint.RowPitch, pitch);
    D3D12_RANGE empty = { 0, 0 };
    state.readback->Unmap(0, &empty);
    return true;
}

static int RunWanGPVideo()
{
    ...
    for (uint32_t index = 0; index < (std::max)(60u, header.warmup); ++index) { PumpPresent(true); Sleep(8); }
    int flags = NVSDK_NGX_DLSS_Feature_Flags_MVLowRes | NVSDK_NGX_DLSS_Feature_Flags_AutoExposure | NVSDK_NGX_DLSS_Feature_Flags_DepthInverted;
    if (header.flags != 0) flags = static_cast<int>(header.flags);
    NVSDK_NGX_Result create_result = NVSDK_NGX_Result_Fail;
    const bool upscale = header.output_width != header.width || header.output_height != header.height;
    bool created = !upscale || PickSrQuality(header.width, header.height, header.output_width, header.output_height);
    created = created && CreateFeature(header.width, header.height, flags, &create_result, header.output_width, header.output_height);
    setup.status = created ? 0u : static_cast<uint32_t>(create_result);
    if (!WriteStdio(&setup, sizeof(setup)) || !created) return 4;

    const size_t pixels = static_cast<size_t>(header.width) * header.height;
    std::vector<BYTE> color(pixels * 4), motion(pixels * 4), depth(pixels * 4), output;
    for (uint32_t index = 0; index < header.frame_count; ++index)
    {
        WanGPFrameHeader frame = {};
        if (!ReadStdio(&frame, sizeof(frame)) || frame.magic != WANGP_FRAME_MAGIC || !ReadStdio(color.data(), color.size()) ||
            !ReadStdio(motion.data(), motion.size()) || !ReadStdio(depth.data(), depth.size())) return 5;
        PumpPresent(true);
        const bool ok = UploadWanGPFrame(state, color.data(), motion.data(), depth.data()) &&
            Evaluate(state.color.texture, state.output, state.depth.texture, state.motion.texture, state.width, state.height, frame.reset != 0, 1.0f, 1.0f) &&
            DownloadWanGPFrame(state, output);
        WanGPFrameResult result = { WANGP_FRAME_OUT_MAGIC, frame.index, ok ? 1u : 0u, ok ? static_cast<uint32_t>(output.size()) : 0u,
                                    ok ? 1u : static_cast<uint32_t>(NVSDK_NGX_Result_Fail), frame.pts };
        if (!WriteStdio(&result, sizeof(result)) || (ok && !WriteStdio(output.data(), output.size()))) return 6;
        if (!ok) return 7;
    }
    return 0;
}
    ...
    for (;;)
    {
        ...
        if (tag == 'B')
        {
            ...
            if (host_creates_b)
            {
                ...
                // game, which imports them (PLAN-OPENGL Â§5 design A; PLAN-VULKAN32 Â§2).
...
{
    bool wangp_video = false;
    for (int i = 1; i < argc; ++i)
        if (strcmp(argv[i], "--wangp-video") == 0) wangp_video = true;
    g_stdio_mode = wangp_video;
    if (wangp_video)
    {
    ...
    const int rc = test ? RunTest() : wangp_video ? RunWanGPVideo() : Serve(pid);
    ShutdownDisguise();
